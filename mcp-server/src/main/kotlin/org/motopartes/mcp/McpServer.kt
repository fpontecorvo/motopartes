...
Retorna lista con id, codigo, nombre, precios y stock."""
    ) { request ->
...
Retorna productos ordenados por stock descendente con valor total."""
    ) { _ ->
        ...
        val text = if (products.isEmpty()) "No hay productos con stock disponible."
            ...
            "Productos con stock: ${products.size} (${totalUnits} unidades, valor: \$${totalValue} ARS)\n" +
...
Usar cuando ya se tiene el ID (de search_products)."""
  ...
  Compra: ${p.purchasePrice} ${p.purchaseCurrency} | Venta: $${p.purchasePrice} ARS | Stock: ${p.stock}""")))
    }

    server.addTool(
        name = "create_product",
        description = """Crear un nuevo producto. Parametros: code, name, purchasePrice (string numerico),
purchaseCurrency (USD o ARS), """
    ) { request ->
        try {
            ...
            val product = productRepo.insert(Product(code = code, name = name, purchasePrice = BigDecimal(pp), purchaseCurrency = Currency.valueOf(pc.uppercase())))
...
Campos opcionales: code, name, purchasePrice, purchaseCurrency, """
    ) { request ->
        try {
...
Dejar status vacio para todos."""
        ...
        val text = if (orders.isEmpty()) "No hay pedidos${if (status.isNotBlank()) " ($status)" else ""}."
            ...
            val cn = try { orderRepo.clientName(it.clientId) } catch (_: Exception) { null }
            "Pedido #${it.id} | ${cn ?: "cliente:${it.clientId}"} | ${it.status} | \$${it.totalArs} ARS | ${it.createdAt.toString().take(16)}"
        }
        CallToolResult(content = listOf(TextContent(text)))
    }
    ...
    server.addTool(
        ...
        val cn = try { orderRepo.clientName(o.clientId) ?: "id=${o.clientId}" } catch (_: Exception) { "id=${o.clientId}" }
        ...
        CallToolResult(content = listOf(TextContent("Pedido #${o.id} | $cn | ${o.status} | ${o.createdAt}\nItems:\n$items\nTOTAL: \$${o.totalArs} ARS")))
    }
...
Usar cuando preguntan "como va el negocio", "resumen", "estado general"."""
        ...
        val totalUnits = products.sumOf { it.stock }
        val totalValue = withStock.sumOf { BigDecimal(it.stock.toLong()) * it.purchasePrice }
        val clients = clientRepo.findAll()
        val totalDebt = clients.sumOf { it.balance }
...
Valor stock: $${totalValue} ARS${dollar?.let { " (~USD ${(totalValue / it.rate).setScale(0, java.math.RoundingMode.HALF_UP)})" } ?: ""}
